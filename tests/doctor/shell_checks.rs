...
fn test_path_configuration_missing_shims() {
    ...
    let original_path = env::var("PATH").unwrap_or_default();
    let shims_path = config.kopi_home().join("shims").to_string_lossy().to_string();
    let separator = if cfg!(windows) { ";" } else { ":" };
